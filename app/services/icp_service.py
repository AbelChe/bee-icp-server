...
class ICPService:
    ...
    async def search_by_domain(self, db: Session, domain: str, force_refresh: bool = False, include_history: bool = False) -> List[ICPRecordBase]:
        ...
        if is_gov_domain(normalized_domain):
            return await self._search_gov_domain(db, normalized_domain, force_refresh, include_history)
        else:
            return await self._search_regular_domain(db, normalized_domain, force_refresh, include_history)
